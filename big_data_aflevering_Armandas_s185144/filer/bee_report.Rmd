...
date: 22/12 2020  \pagebreak
bibliography: bibliography.bib
csl: university-college-lillebaelt-apa.csl
output:
  pdf_document:
    toc: yes
    toc_depth: 3
...
header-includes:
  ...
  - \pretitle{\begin{center}
    \includegraphics[width=2in,height=2in]{ic_beeware_launcher_logo-web.png}\LARGE\\}
...
Der er i alt 43 søjler i datasættet, men ud fra opsummering af variabler i [Bilag A](#bilag_A) kan man konstatere at disse variabler kun er i brug og relevante: `hive_observation_time_local`, `hive_weight_kgs`, `hive_temp_c`, `hive_humidity`, `ambient_temp_c`,
...
par(mar = c(5, 5, 3, 5))
plot(hive3$hive_observation_time_local, hive3$hive_weight_kgs, type ="l", ylab = "Vægt(kg)",
...
plot(hive1$hive_observation_time_local, hive1$daily_weight_delta, type="h",main="Daglig vægttilvækst", ylab="Vægt(kg)", xlab="", col="red")
...
Modellen som blev lavet i forudgående afsnit blev brugt i `forecast` funktionen [@hyndman]  til at forudsige vægten i hele marts måned, altså 31 dage. På Figur \ref{fig:forecast} nedenfor kan man se de forudsagte værdier. En blå linje betyder "Point Forecast", den mørke gråzone markerer 80% konfidensinterval, og den lyse gråzone markerer 95% konfidensinterval. 
...
plot(forecast, ylab = "Vægt(kg)", xaxt="n", col="red", main="Forudsigelse af vægtudvikling på Bistade1")
...
For det andet blev der også undersøgt modellens `Root mean squared error`(`RMSE`), som er udregnet ved $\sqrt{(mean(e_t^2))}$ [@hyndman_athanasopoulos (afsnit 3.4)]. I outputtet nedenfor kan man se at modellen i forhold til målestok har rimeligt lavt `RSME` værdi[@astur], som er $0.077$. 
...
- Vægttilvæksten, som oprindeligt var kvantitativ variabel, blev lavet om til kvalitative variablel. Vægttilvæksten blev opdelt i to kategorier, nemlig UP og DOWN. UP er når vægtilvæksten er positiv og DOWN er når vægttilvæksten er negativ. 
...
Disse forberedelser på træningssættet kan man se i R kodeudklippet i [Bilag C](#bilag_C). 

```{r echo=FALSE ,  fig.cap="\\label{fig:weather_data} Data fra bistadet sammensat med eksternt vejrdata fra DMI i juni måned 2020.", fig.height=8.5}
...
hive_data_2020_jun <- import_hive_data_daily(from = "'2020-06-01 00:00:00'",
                                                   to="'2020-07-01 00:00:00'")
hive_data_2020_jun_merged_weather <- merge(hive_data_2020_jun,
                                  weather_data_furesoe_jun, by="dt") 
hive_data_2020_jun_merged_weather <- hive_data_2020_jun_merged_weather %>% rename(hive_weight_kg = hive_weight_kgs_daily,  ambient_temp_c=ambient_temp_c_day_max)
plot.ts(hive_data_2020_jun_merged_weather[-1], nc=1 , yax.flip = TRUE, main="")
...
For at klassificere om vægttilvækst skulle være opadgående eller nedadgående, blev der valgt at bruge beslutningstræer, fordi træer er nemmere at fortolke og at visualisere på grafen end de andre modeller bl.a. linær regression eller logistisk regression. På en anden side kan beslutningstræer være mindre præcise og de er heller ikke så robuste, da små ændringer i datasættet kan medføre stor ændringer i den endelig resultat. [@soerensen]
...
I R koden nedenfor kan man se at der blev brugt `ctree` funktion[@hothorn] til at lave "Conditional inference tree". `weight_delta_direction` er sat til at være "outcome variable" i modellen, og alle resterende variabler i datasættet, altså `ambient_temp_c_day_max`, `precipitation_mm`,	`wind_m_s`,	`drought_index`,	`humidity_proc`, `pressure_hPa` og	`sun_hours`
, skulle bruges som "predictors". 
...
<!--
De største overraskelse i træet er, at luftryk er en af de mest signifikant parametre, men dette kan muligvis forklares, at bierne forudsiger et regnvejr, hvis luftryk er faldende. 

Det har dog også observeret flere andre biavler, at biene ikke flyver ud så meget, når luftryk falder, da det kan betyde stormen. [^beemaster]    

[^beemaster]:https://beemaster.com/forum/index.php?topic=53294.0

https://ezinearticles.com/?Barometric-Pressure-and-How-It-Affects-Animals&id=6656769#:~:text=Many%20animals%20rely%20on%20scent,the%20ground%20and%20last%20longer.
-->
...
Der blev også eksperimenteret med deep learning til at forudsige vægttilvækst. Der blev benyttet `Keras` API i `Python` til at lave modellen. Hele koden kan findes i [Bilag D](#bilag_D).

I starten var det lidt svært at få noget ud af det, fordi både min træningssæt og valideringsæt indeholder kun 30 rækker hvert. Det er dog anbefalet, at der skal være et stort datasæt, før det overhovedet giver mening at bruge deep learning[@sharma]. 

Til gengæld lykkedes det at lave modellen mere præcist ved at øge `epochs` parameter til 200, dvs. hvor mange gange der skal  køres igennem træningssættet. Det kan forklares med, at hvis der kun er et lille træningssæt og `epochs` heller ikke er stor, så bliver modellen simpelthen ikke trænet nok gange igennem[@beam].
...
```
### Validering af Deep learing


I valideringsfasen blev der brugt datasættet fra 2019 juni måned for at validere, hvor præcist modellen er i virkeligheden og opfange evt. "overfitting". På Firgur \ref{fig:cm_deep_learning} kan man se, at modellen har gættet 23 gange rigtigt ud af 30 forsøg, hvilket svarer til `0.76` præcision. 
...
```
```
1/1 [==============================] - 0s 18ms/step - loss: 0.7000 - accuracy: 0.7333
Test accuracy: 0.7333333492279053
![alt text here](cm_deep_learing.png)
```
-->

```{r fig.width=3.5, fig.height=3.5,echo=FALSE, fig.cap="\\label{fig:cm_deep_learning} Confusion Matrix af de observede værdier og de forudsagde værdier ved at bruge deep learning."}
library(png)
library(grid)
...
Personligt synes jeg også at projektet var givende, da jeg har afprøvet adskillige R og Python værktøjer i praksis. I fremtiden kunne det være dog spændende implementere modellerne i produktionen, så at det rent faktisk kunne bidrage biavlen med at passe på hans bifamilier. Dette var dog ikke en del af dette kursus.  
...
## Bilag B: Mine funktioner i R{#bilag_B} 
```R
...
import_hive_data_daily<- function(from, to){
  ...
  periods_to_remove <- 
  read.table(file="data/stade1_period_to_ignore_manipulate.csv", sep=",", header = TRUE)
  hive_data <- manipulate_weight_deltas(hive_data=hive_data, periods=periods_to_remove)
...
## Bilag C: Dataklargøring til klassificering af vægttilvækst{#bilag_C}
...
## Bilag D: Deep learning i Python {#bilag_D}
...
import numpy as np
hive_data_2019_jun_validate = pd.read_csv('hive_data_2019_jun.validate.csv')
hive_data_2020_jun_train = pd.read_csv('hive_data_2020_jun.train.csv')
